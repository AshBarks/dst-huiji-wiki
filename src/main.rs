...
use dst_huiji_wiki::{DstContext, Error, Result};
...
fn handle_map_names(
    ...
    let wiki_data = if merge {
        let compare_path = compare.as_ref().ok_or_else(|| {
            Error::Config("--merge requires --compare".to_string())
        })?;
...
async fn maintain_recipe_builder_tag_lookup(output: Option<PathBuf>) -> Result<()> {
    ...
    let target_content = page.content.ok_or_else(|| {
        Error::WikiApi("Wiki page has no content".to_string())
    })?;

    println!("Extracting RECIPE_BUILDER_TAG_LOOKUP from debugcommands.lua...");
    let result = process_copyclip(&debugcommands_string, "RECIPE_BUILDER_TAG_LOOKUP", &target_content)?;
...
fn output_result<T: serde::Serialize + std::fmt::Debug>(data: &[T], output: Option<PathBuf>) -> Result<()> {
    if let Some(output_path) = output {
