...
- **v3.4（2026-08-26 实施记录）**：M2/M3a/M4 前段落地——F1 loot、F2 stats、F3 行为常量子集、fn 标注 MVP、BrainEdge 动态解析验证、atlas 全覆盖关联渲染、F4 SpawnPrefab 反向生成首版、create-check 清单正式化、state.json 缓存断点续跑、Page→Symbol 标注 P0/P1/P2/P3/P4 + `symbol-annotate` CLI runner（SymbolPageAnnotation + 受影响页/证据组装 + 高引用 symbol 证据包 + Prompt/输出契约 + 跨页一致性报告）；join 阶段大小写归一（case_only 不再进人工纠错）、vault_crawler create_check 关闭；§4.2.2 反向传播收窄已实施（组件变更仅传播本地覆写者，超阈值聚合）。
