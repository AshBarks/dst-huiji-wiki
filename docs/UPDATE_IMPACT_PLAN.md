...
- **v3.4（2026-08-26 实施记录）**：M2/M3a/M4 前段落地——F1 loot、F2 stats、F3 行为常量子集、fn 标注 MVP、BrainEdge 动态解析验证、atlas 全覆盖关联渲染、F4 SpawnPrefab 反向生成首版、create-check 清单正式化；join 阶段大小写归一（case_only 不再进人工纠错）、vault_crawler create_check 关闭；§4.2.2 反向传播收窄已实施（组件变更仅传播本地覆写者，超阈值聚合）。
...
| 同一 prefab 的**条件语境展开为分句枚举** | "完全野生仇恨 100 / 有丘 20 / 海象营地寒冰猎犬 10 单位"三行并列 | FactChange 增加 `context` 维度，各语境值独立匹配、互不误伤 |
...
| M4 F3/F4+打磨 | behavior/backref、**BrainEdge 动态解析（require 追踪/参数流/常量折叠）**、取舍配置回填、create-check、缓存断点续跑 | 🟨 F3/BrainEdge/atlas/F4 首版/create-check 清单化完成；缓存断点续跑待做 |
