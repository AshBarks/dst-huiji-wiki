...
pub fn collect_brain_consts(root: &Path) -> Result<Vec<ConstFact>> {
    ...
    let mut out = Vec::new();
    let re = match Regex::new(r"^\s*local\s+([A-Za-z_]\w*)\s*=\s*(\d+(?:\.\d+)?)\s*$") { Ok(r) => r, Err(e) => return crate::Result::Err(crate::error::Error::Config(format!("consts regex: {e}"))) };
    let Ok(entries) = std::fs::read_dir(&dir) else { return Ok(out) };
    for e in entries.flatten() {
        let p = e.path();
        if p.extension().map(|x| x != "lua").unwrap_or(true) { continue; }
        ...
        for (i, line) in src.lines().enumerate() {
            ...
            if let Some(c) = re.captures(line) {
                if let Ok(v) = c[2].parse::<f64>() {
                    out.push(ConstFact { name: c[1].to_string(), value: v, line: (i + 1) as u32, file: file.clone() });
                }
            }
        }
    }
...
pub fn pair_const_changes(
    old: &[ConstFact],
    new: &[ConstFact],
) -> Vec<FactChange> {
    let mut m: BTreeMap<(String, String), (Option<f64>, Option<f64>, String, u32)> = BTreeMap::new();
    for f in old { m.entry((f.file.clone(), f.name.clone())).or_insert((None, None, f.file.clone(), f.line)).0 = Some(f.value); }
    for f in new { m.entry((f.file.clone(), f.name.clone())).or_insert((None, None, f.file.clone(), f.line)).1 = Some(f.value); }
    let mut out = Vec::new();
    for ((file, name), (o, n, f, l)) in m {
        if o == n { continue; }
        let prefab = file.trim_end_matches(".lua").rsplit('/').next().unwrap_or("?").to_string();
        out.push(FactChange {
            prefab,
...
mod tests {
    ...
    fn temp_brain(src: &str) -> (std::path::PathBuf, std::path::PathBuf) {
        let dir = std::env::temp_dir().join(format!("f3-{}-{}", std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
    ...
    fn extracts_consts_and_skips_comments_and_strings() {
        let (dir, _f) = temp_brain("-- local SEED = 1\nlocal SEE_DIST = 30\nlocal NAME = \"x\"\nlocal RATE = 0.5\n");
    ...
    fn pairing_fires_only_on_value_change() {
        ...
        let new = vec![ConstFact { name: "SEE_DIST".into(), value: 32.0, line: 1, file: "brains/houndbrain.lua".into() }];
